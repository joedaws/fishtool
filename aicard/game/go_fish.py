from aicard.game.events import BookEvent, DrawEvent, ExchangeEvent, AskEvent
from aicard.game.states import GoFishState
from aicard.game import GO_FISH_INITIAL_HAND_SIZE_MAP
...
class GoFishGame:
    ...
    Note: 
        ...
        a map which maps a play index to a list of indices of that player's
        opponents.
    """
    ...
    def player_turn(self, player):
        ...
        # If player holds no cards, they may draw a card if there are cards remaining in the deck.
        if len(player.hand) == 0:
            if len(deck.cards) > 0:
                player.draw(deck)

        # a player may ask for a card if they have at least one card.        
        keep_asking = len(player.hand) > 0

        # a player may keep asking for cards as long as they recieve a card from an opponent.
        asks = 0
        while keep_asking:
            ...
            ask_index = action[0]  # index of player to ask for a card.
